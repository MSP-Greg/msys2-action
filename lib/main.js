...
async function openssl() {
  if (rubyABIVers >= '2.5') {
    ...
    const openssl_2_4 = `https://dl.bintray.com/larskanis/rubyinstaller2-packages/${prefix.trim()}openssl-1.0.2.s-1-any.pkg.tar.xz`;
...
async function updateGCC() {
  ...
  //let gccPkgs = ['', 'binutils', 'crt', 'headers', 'isl', 'libiconv', 'mpc', 'windows-default-manifest', 'libwinpthread', 'winpthreads', 'zlib', 'gcc-libs', 'gcc'];
  let gccPkgs = ['', 'binutils', 'isl', 'libiconv', 'mpc', 'windows-default-manifest', 'libwinpthread', 'winpthreads', 'zlib', 'gcc-libs', 'gcc'];
